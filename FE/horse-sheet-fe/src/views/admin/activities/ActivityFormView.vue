...
async function loadActivity() {
  try {
    ...
    uiStore.showError(error.message || 'Failed to load activity');
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      uiStore.showSuccess('Activity updated successfully');
    } else {
      await activityService.create(form.value);
      uiStore.showSuccess('Activity created successfully');
    }
    router.push('/admin/activities');
  } catch (error: any) {
    if (error.status === 409) {
      uiStore.showError('This activity has been modified by another user. Please refresh and try again.');
      if (isEdit.value) await loadActivity();
    } else if (error.errors) {
      errors.value = error.errors;
    } else {
      uiStore.showError(error.message || 'Failed to save activity');
    }
  } finally {
  ...
  <div class="activity-form">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">{{ isEdit ? 'Edit Activity' : 'Create New Activity' }}</h2>
        <router-link to="/admin/activities" class="btn btn-secondary">Back to List</router-link>
      </div>
      <div v-if="loading" class="loading"><div class="spinner"></div></div>
      <form v-else @submit.prevent="handleSubmit">
        <div class="form-group">
          <label class="form-label required" for="name">Name</label>
          <input id="name" v-model="form.name" type="text" class="form-input" :class="{ 'has-error': errors.name }" placeholder="Enter activity name" />
          <span v-if="errors.name" class="form-error">{{ errors.name }}</span>
        </div>
        <div class="form-group">
          <label class="form-label" for="description">Description</label>
          <textarea id="description" v-model="form.description" class="form-textarea" placeholder="Enter description"></textarea>
        </div>
        <div class="form-group">
        ...
        <div class="form-actions">
          ...
          <router-link to="/admin/activities" class="btn btn-secondary">Cancel</router-link>
        </div>
      </form>
    </div>
  </div>
