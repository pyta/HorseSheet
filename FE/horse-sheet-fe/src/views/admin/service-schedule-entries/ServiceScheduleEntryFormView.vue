...
function validate(): boolean {
  ...
  if (!form.value.duration?.trim()) {
    errors.value.duration = t('validation.duration.required');
    return false;
  }
  ...
  <div class="service-schedule-entry-form">
    <div class="card">
      <div class="card-header">
        <h2 class="card-title">{{ isEdit ? t('schedule.serviceEntry.editTitle') : t('schedule.serviceEntry.createTitle') }}</h2>
        <router-link to="/admin/service-schedule-entries" class="btn btn-secondary">{{ t('common.buttons.backToList') }}</router-link>
      </div>
      ...
      <form v-else @submit.prevent="handleSubmit">
        ...
        <div class="form-group">
          <label class="form-label required" for="duration">{{ t('common.labels.duration') }}</label>
          <input id="duration" v-model="form.duration" type="text" class="form-input" :class="{ 'has-error': errors.duration }" :placeholder="t('common.labels.durationExample')" />
